...
- `package.json` defines npm scripts only (`build`, `dev`, `preview`). There are **no npm dependencies**; Node.js 24+ is required only to run the build script. `npm run preview` uses `npx serve` on demand.
...
- `templates/`. HTML source for the landing page.
  ...
  - `templates/root-redirect.html`. Minimal HTML served at `dist/index.html` to redirect `/` to the default locale (`/en/`).
...
- `build.mjs`. Build pipeline: load locales, validate i18n parity, render each section through the template engine, assemble `layout.html`, write `dist/<lang>/index.html`, copy `public/`, write `dist/index.html` (redirect to `/en/`).
...
- `dist/index.html` — redirect to the default locale (`/en/`).
